...
export function ReportView({
  ...
  React.useEffect(() => {
    if (!reportId || internalStatus === "COMPLETED" || internalStatus === "FAILED") {
      return;
    }
  ...
  if (isLoading || (internalStatus && internalStatus.startsWith("PROCESSING"))) {
    return <ProcessingView status={internalStatus} filename={filename} createdAt={createdAt} />;
  }

  if (!internalReportContent) {
    return (
