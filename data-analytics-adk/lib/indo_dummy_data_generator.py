...
# CONFIGURATION
# ==========================================
NUM_OF_ROWS = 10_000_000  # Adjusted for demo, set to 15M as needed
QUALITY_TYPE = 'clean' 
...
def generate_sales_data_id():
    ...
    # Prices in IDR (Approximate market values)
    catalog = [
      # Elektronik (12 items)
    ...
    # Realistic IDR variance (rounding to nearest 100 or 500 is common in ID)
    df['unit_price'] = (catalog_prices[indices] * np.random.uniform(0.95, 1.05, NUM_OF_ROWS))
    df['unit_price'] = (df['unit_price'] // 100) * 100  # Round to nearest 100
    ...
    if QUALITY_TYPE == 'dirty':
        ...
        # B. Outliers (e.g., someone typing an extra zero)
        ...
        # C. Negative Values (Return/Refund errors)
        ...
        # D. Inconsistent Text (Common ID typos/abbreviations)
        ...
        dirty_city_map = {
            ...
            'Bandung': 'BDG',
            'Surabaya': 'SBY',
            'Makassar': 'makasar' # Typo (missing one 's')
        }
    ...
    # Ensure directory exists or remove path for testing
